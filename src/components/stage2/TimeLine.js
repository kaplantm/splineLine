...
import {mapNumericValueToRange,
    getColorCustomOpacityFromHSLA} from '../../utils/generalUtils';
...
function TimeLine(props) {
    ...
    for (let i = 0; i < lineCount; i++) {
        ...
        for (let j = 0; j < cellsPerLine; j++) {
            ...
            cells.push(<React.Fragment key={`TIMELINE_CELL_${i + j}`}>
                {element}
            </React.Fragment>);
        }
        lines.push(<React.Fragment key={`TIMELINE_CENTER_${i}`}>

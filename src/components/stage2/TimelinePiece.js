...
function TimeLinePiece(props) {
    const {style = {},
        ...
        appMode,
        cellId,
        defaultState = Modes.VIEW_MODE} = props;

    const [modeValue, setMode] = useState(defaultState);
    const [savedContentValue, setSavedContent] = useState(window.localStorage.getItem(`savedContent${cellId}`));
    const [savedImageValue, setSavedImage] = useState(window.localStorage.getItem(`savedImage${cellId}`));
    const [savedLabelValue, setSavedLabel] = useState(window.localStorage.getItem(`savedLabel${cellId}`));
    ...
    const state = {mode: { updater: setMode, value: modeValue },
        ...
        hoverText: {updater: setHoverText,
            value: hoverText}};
    ...
    function getPieceChildContent() {
        return (appMode.value === 'view' || state.mode.value === Modes.VIEW_MODE) ? (
            <React.Fragment>
    ...
    state.content.value ||
    state.label.value ||
    (appMode.value === 'edit' && state.mode.value === Modes.EDIT_MODE);
